  ...
  while (choice != 8) {
    ...
    case 1: {
      ...
      if (r > 0 && c > 0) {
        ...
        cout << "Матрица создана под индексом " << matrices_count << endl;
        matrices_count++;
      } else {
    ...
    case 6: {
      ...
      if (index >= 0 && index < matrices_count && matrices[index]) {
        int r, c;
        double val;
        cout << "Введите номер строки (с 0), столбца (с 0) и новое значение: ";
        cin >> r >> c >> val;
        if (r >= 0 && r < matrices[index]->GetRows() && c >= 0 &&
            c < matrices[index]->GetCols()) {
          matrices[index]->GetMatrix()[r * matrices[index]->GetCols() + c] =
              val;
