...
// Деструктор
template <class T> matrixes<T>::~matrixes() {
  if (matrix) {
    delete[] matrix;
    matrix = nullptr;
  }
...
template <class T> T matrixes<T>::GetIJ(int i, int j) const {
  return matrix[cols * i + j];
}

template <class T> T matrixes<T>::Get(int index) const { return matrix[index]; }

template <class T> T *matrixes<T>::GetMatrix() const { return matrix; }

template <class T> int matrixes<T>::GetRows() const { return rows; }

template <class T> int matrixes<T>::GetCols() const { return cols; }

// Заполняет матрицу значениями матрицы Гильберта
template <class T> void matrixes<T>::MakeGilbert() {
  if (rows != cols) {
    return;
  }
...
template <class T> T *matrixes<T>::GetInverseMatrix() const {
...
template <class T> void matrixes<T>::MakeOnes() {
  if (rows != cols) {
    return;
  }
...
template <class T> void matrixes<T>::MakeRandomNormal() {
...
template <class T> T matrixes<T>::GetDet() const {
  if (rows != cols) {
