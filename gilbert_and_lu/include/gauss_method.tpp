...
template <class T> gauss_method<T>::gauss_method(T *m, int n) {
  matrix = new T[n * n];
  conmatrix = new T[n * n];
  this->n = n;
  this->det = T(1.0);
...
template <class T> T *gauss_method<T>::Solve(T *b) {
  for (int i = 0; i < n; i++) {
    conmatrix[n * i] = b[i];
  }
...
template <class T> void gauss_method<T>::SolveForTests(T *b) {
  for (int i = 0; i < n; i++) {
    conmatrix[n * i] = b[i];
  }
...
template <class T> T *gauss_method<T>::GetMatrix() const {
  return this->conmatrix;
}
